...
const Buy = ({ showBuy, onClose, fiat: { fiat }, coin, c, buyCurrency, getFiat, getCoins, updateFiat }) => {
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title"
              style={{ color: 'green' }}
            >
              Buy {c.name.charAt(0).toUpperCase() + c.name.slice(1)}...?</h5>
          </div>
                ...
                <tr>
                  ...
                  <td>
                    <select onChange={(e) => setChosenFiat(e.target.value.toLocaleLowerCase())}>
                      <option disabled>chose Fita curr</option>
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={(e) => buying()}>Buy</button>
          </div>
        </div>
      </div>
    </div>);
